...
# You must include either the 'ip' key OR use 'role' and/or 'environment' keys to lookup the valid ips
  ...
  if v.has_key?('firewall') && v['firewall'].has_key?('type')
    ...
    v['firewall']['destinations'].each do |d|
      ...
      if d.has_key?('ip')
        ...
        rule['ip'] = d['ip']
        rules << rule.dup
        log "creating firewall rule to #{permission} traffic from #{source_cidr} to #{ip}#{port_msg}"
      else # need to lookup the ip(s) using chef search
        if d.has_key?('role')
          query += "roles:#{d['role']}"
        end
    ...
    log "Invalid firewall specification for vpn daemon: #{k}"
  end
end
